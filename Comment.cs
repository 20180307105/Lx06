...
namespace DAL
{
    ...
    public class Comment
    {
        ...
        public IEnumerable<Model.CommentNo> GetWorkPage(Model.CommentPage page)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by commentTime desc) as num,Comment.* from Comment where Comment.workId=@worlId)";
                sql += "select * from a where num between (@pageIndex-1)*@pageSize and @pageIndex*@pageSize;";
                return cn.Query<Model.CommentNo>(sql, page);
            }
        }
        ...
        public int Update(Model.Comment comment)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update Comment set CommentContent=@CommentContent,CommmentTime=@CommentConetnt where commentID=@commentID";
